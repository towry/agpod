...
impl AgpodMcpServer {
    #[tool(
        name = "hive",
        description = "Manage tmux-backed Claude exec workers derived from the current tmux pane. `spawn_agent` only registers a reusable worker profile; `send_prompt` lazily creates or reuses the pane-derived tmux session and starts the run. Use `mode_info`, `spawn_agent`, `list_agents`, `send_prompt`, `close_agent`, or `close_session`. Public `mode` names are fixed to `readonly` and `full`; missing mode config is an error. `~` in configured paths is expanded to the home directory.",
