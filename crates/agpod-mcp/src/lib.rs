...
impl AgpodMcpServer {
    #[tool(
        name = "hive",
        description = "Manage reusable worker agents in a repo-scoped default session, or an explicit `session_id` when provided. Use `mode_info`, `run_hive_agent`, `list_agents`, `close_agent`, or `close_session`. `mode_info` lists the public modes and whether each is currently available, `run_hive_agent` creates or reuses one worker and starts one run, `list_agents` reports session or agent state, `close_agent` closes one worker, and `close_session` closes the whole hive session. `run_hive_agent` blocks until completion unless `async=true`; when async, poll `list_agents` with `agent_id`. Public `mode` names are `readonly` and `full`. `resume=true` requires a previously saved conversation for that worker and fails fast if none is available.",
...
mod tests {
    ...
    fn tool_router_exposes_case_tools() {
        ...
        assert!(instructions.contains(
            "`mode_info` lists the public modes and whether each is currently available"
        ));
        assert!(instructions.contains("Public `mode` names are `readonly` and `full`"));
